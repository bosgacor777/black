...
class BlackjackGame {
  ...
  _applyServerState(state) {
    ...
    // Update action panel berdasarkan fase
  ...
  _handleOnlineDeal(events) {
    ...
    // Animasi kartu satu per satu dengan delay
  ...
  _handleOnlineRoundResult(data) {
    ...
    if (myResult) {
      ...
      // Pick best result dari semua hand
      ...
      // Show result overlay (delay sedikit untuk animasi kartu)
      setTimeout(() => {
        this.UI.Result.show({
          result     : bestResult,
          headline   : phrase.h,
          ...
          onContinue : () => { /* nothing, server akan trigger new_round_ready */ }
        });
      }, 1500);
    }
  }
  ...
  _enterGameScreen(mode) {
    ...
    // Setup game action buttons
    this._setupGameButtons();

    // Chat — hanya tampil di online
    if (mode === 'online') {
    ...
    // Leave button
    this.UI.DOM.btnLeave.addEventListener('click', () => this._handleLeave());
  ...
  _addToBet(amount) {
    ...
    // Animasi chip terbang ke display
    const chipBtn     = event.currentTarget;
  ...
  async _confirmBet() {
    ...
    try {
      if (this.mode === 'online') {
        ...
        // Otomatis deal di offline setelah bet
        setTimeout(() => {
          try {
            this.engine.deal();
          } catch (err) {
  ...
  _disableActionsTemp() {
    ...
    // Re-enable setelah response dari server / engine
    setTimeout(() => this._reenableActions(), 800);
  }

  _reenableActions() {
    // Hanya enable jika masih di fase player turn & giliran kita
    if (this.mode === 'online') return; // Online dihandle oleh state update
...
function _pickBestResult(hands) {
  ...
  // Prioritas: BLACKJACK > WIN > PUSH > BUST > LOSE
...
(async function boot() {
  ...
  // Safety check — semua modul harus tersedia
  if (!window.UI) {
  ...
  window._game = game; // Expose untuk debugging
